...
Kinematic::rps Kinematic::calculateRPS(float linear_x, float linear_y, float angular_z, float imu_angular_z)
{
    ...
    // front-left motor
    float rps_motor1 = -sin(toRad(135 + imu_angular_z)) * x_mps + cos(toRad(135 + imu_angular_z)) * y_mps + robot_radius_ * tan_mps;
    rps.motor1 = fmax(-max_rps_, fmin(rps_motor1, max_rps_));

    // rear-left motor
    float rps_motor2 = -sin(toRad(225 + imu_angular_z)) * x_mps + cos(toRad(225 + imu_angular_z)) * y_mps + robot_radius_ * tan_mps;
    rps.motor2 = fmax(-max_rps_, fmin(rps_motor2, max_rps_));

    // rear-right motor
    float rps_motor3 = -sin(toRad(315 + imu_angular_z)) * x_mps + cos(toRad(315 + imu_angular_z)) * y_mps + robot_radius_ * tan_mps;
    rps.motor3 = fmax(-max_rps_, fmin(rps_motor3, max_rps_));

    // front-right motor
    float rps_motor4 = -sin(toRad(45 + imu_angular_z)) * x_mps + cos(toRad(45 + imu_angular_z)) * y_mps + robot_radius_ * tan_mps;
